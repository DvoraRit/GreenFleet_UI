...
function ExtrasTable({ onChange, initialValues }) {
	...
	const [lines, setLines] = useState([
		{
			...
			extention_type: { label: 'extention_type', value: '', width: '18%' },
			extension: { label: 'extension', value: '', width: '20%' },
			selected_value_type: {
				label: 'selected_value_type',
				value: '',
				width: '20%',
			},
			extention_value: { label: 'extention_value', value: '', width: '35%' },
		},
	]);
	...
	useEffect(() => {
		if (initialValues && initialValues.extras?.length > 0) {
			let newLines = [];
			initialValues.extras?.forEach((element) => {
				let obj = JSON.parse(JSON.stringify(lines[0]));
				Object.values(obj).forEach((item) => {
					item.value = element[item.label];
					if (element['extention_type'] !== '') {
						setExtentionOptions(ExtentionsTypes[element['extention_type']]);
					}
